...
export async function POST(req: NextRequest) {
    const { targetUrl, text, workspaceId } = await req.json();
    if (!targetUrl) {
        return NextResponse.json({
            error: 'URL parameter is required'
        }, { status: 400 });
    }
    const checkWorkspace = await db.workspace.findFirst({
        where: {
            id: workspaceId
        }
    });
    if (!checkWorkspace) {
        return NextResponse.json({
            ...
            details: 'The provided workspace ID does not exist in the database.'
        }, { status: 401 });
    }
