...
const WorkspacePage2 = ({ workspace }: { workspace: any }) => {
    ...
    const init = async () => {
        ...
        let createdFiles = Object.keys(formattedFiles);
    ...
    const getAiResponse = async () => {
        ...
        const newMessages: Message[] = [
            ...
            {
                role: "assistant",
                content: result.explanation + (result?.generatedFiles.length > 0 ? "\nFiles Created:\n" + result?.generatedFiles.join("\n") : "") + (result?.editedFiles.length > 0 ? "\nFiles Modified:\n" + result?.editedFiles.join("\n") : "") + (result?.deletedFiles.length > 0 ? "\nFiles Deleted:\n" + result?.deletedFiles.join("\n") : "")
            },
        ]
            ...
            <div className='grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-10'>
                ...
                <div className='md:col-span-1 lg:col-span-2'>
                    <Tabs defaultValue="code" className="h-screen border-2 rounded-lg">
                        ...
                        <TabsContent value="code" className="m-0 h-[calc(100vh-4rem)]">
