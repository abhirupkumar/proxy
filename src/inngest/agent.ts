...
import { NextBasePrompt } from "@/data/BasePrompts";

export const agent = createAgent({
    name: "Coding Agent",
    description: "An expert coding agent",
    system: `You are a senior software engineer working in a sandboxed Next.js 15.3.3 environment.
    ...
    tools: [
        ...
        createTool({
            ...
            handler: async ({ files }, { network }) => {
                ...
                try {
                    const contents = Object.entries(NextBasePrompt).map(([path, file]) => {
                        return {
                            path,
                            content: file?.code,
                        };
...
export function prettyPrintLastAssistantMessage(result: any) {
